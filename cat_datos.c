...
/**
 * ====================================================================================
 * COMANDO: d_append <archivo> <texto>
 * ====================================================================================
 * Demuestra cómo agregar contenido a un archivo existente SIN destruir lo que ya tenía.
 * Es casi idéntico a d_create, pero cambia una sola bandera de open().
 *
 * Syscalls explicadas:
 * 1. open(2): Solicita al kernel abrir (o crear, si no existe) un archivo.
 *    - O_WRONLY: Abre el archivo solo para escritura.
 *    - O_CREAT: Si el archivo no existe, lo crea (igual que d_create).
 *    - O_APPEND: A diferencia de O_TRUNC, NO borra el contenido previo.
 *      Además, garantiza que cada write() se posicione automáticamente
 *      al final del archivo antes de escribir, de forma atómica -esto
 *      evita condiciones de carrera si varios procesos escriben a la vez
 *      (por ejemplo, un archivo de log compartido).
 * 2. write(2): Escribe los bytes nuevos, siempre al final gracias a O_APPEND.
 * 3. close(2): Libera la entrada en la tabla de descriptores de archivos del proceso.
 */
int cmd_d_append(int argc, char **argv) { //declaramos la creación de la función
    if (argc != 3) {
        fprintf(stderr, COLOR_ERROR "Uso: d_append <archivo> <texto>\n" COLOR_RESET);//creamos como el programa debería de leer la función
        return 1;
    }
    const char *filename = argv[1];
    const char *text = argv[2];
    ...
    /* 1. LLAMADA AL SISTEMA: open (con O_APPEND en vez de O_TRUNC) */
    LOG_SYSCALL("open", "\"%s\", O_WRONLY|O_CREAT|O_APPEND, 0644", filename);//que banderas usará la función y la callsystem
    ...
    printf(COLOR_RESULT "Se agregaron %zd bytes al final de '%s'.\n" COLOR_RESET, bytes_written, filename);
    return 0;
}
